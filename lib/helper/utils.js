  ...
  arrayCompare(a, b, isNotEquals) {
    ...
    for (var i in a) {
      if (a[i] instanceof Array) {
        ...
        exports.mes.push(`Field ${i}. Value ${a[i]} !== ${b[i]}`);
        return false;
      } else if (isNotEquals && a[i] === b[i]) {
        exports.mes.push(`Field ${i}. Value ${a[i]} === ${b[i]}`);
        return true;
  ...
  objectCompare: (a, b, isNotEquals) => {
    ...
    for (var i in a) {
      if (a[i] instanceof Array) {
        ...
        if (!isNotEquals && !exports.objectCompare(a[i], b[i], isNotEquals)) {
          exports.mes.push(`Key ${i}. Value: ${JSON.stringify(a[i])} !== ${JSON.stringify(b[i])}`);
          return false;
        } else if (isNotEquals && exports.objectCompare(a[i], b[i], isNotEquals)) {
          exports.mes.push(`Key ${i}. Value: ${JSON.stringify(a[i])} === ${JSON.stringify(b[i])}`);
          return true;
  ...
  ignoreUndefined: (obj) => {
    var removeInObject = (obj) => {
      for (var i in obj) {
        ...
        } else if (typeof obj[i] === 'function' && obj[i].name instanceof Remove.name) {
          delete obj[i];
        }
      }
      return obj;
