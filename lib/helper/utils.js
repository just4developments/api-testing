...
exports = module.exports = {
  ...
  objectCompare: (a, b, isNotEquals) => {
    ...
    for (var i in a) {
      if ('function' === typeof b[i]) {
        if (!isNotEquals && !exports.typeCompare(a[i], b[i])) {
          exports.mes.push(`${i} is NOT instanceof ${b[i].name}${b[i].more || ''}`);
          b[i] = `$(${b[i].name}${b[i].more || ''})`;
          return false;
        } else if (isNotEquals && exports.typeCompare(a[i], b[i])) {
          exports.mes.push(`${i} is instanceof ${b[i].name}${b[i].more || ''}`);
          b[i] = `$(${b[i].name}${b[i].more || ''})`;
          return true;
        }
        b[i] = `$(${b[i].name}${b[i].more || ''})`;
  ...
  typeCompare: (a, b) => {
    ...
    if (type !== RegExp.name) {
      ...
      else if (b === Length && (a instanceof Array || typeof a === 'string')) return b.value === a.length;
            ...
            for (var i in target[key]) {
              ...
              for (var j in source[key]) {
                if (target[key][i] == source[key][j]) {
                  isContains = true;
                  output[key][i] = exports.assign(target[key][i], source[key][j]);
                  break;
                }
              }
              if (!isContains) {
