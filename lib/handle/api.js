  ...
  check(res) {
    if (this.config.checker) {
      for (var checkerObj of this.config.checker) {
        ...
        var preHandlerChecker = (vl) => {
          return vl;
        };
        if (checkerObj["_preHandler"]) {
          preHandlerChecker = checkerObj["_preHandler"];
        }
        checkerFnc(res.code, res.headers, res.body, preHandlerChecker(checkerObj[checkerName]));
