...
class Api {
  ...
  check(res) {
    if (this.config.checker) {
      for (var checkerObj of this.config.checker) {
        ...
        var checkerFnc;
        // try{
        checkerFnc = checker[checkerName];
        if (!checkerFnc) throw { mes: `Checker "${checkerName}" is not defined` };
  ...
  exec(cb0) {
    ...
    var requestBody;
    if (this.config.requestBody !== undefined) this.config.requestBody = Object.assign({}, this.config.requestBody);
    ...
    if ('post' === this.config.method) {
      ...
      if (requestBody !== undefined) {
        if ('form' === transformRequest) handleForm(requestBody);
