...
class Api {
  ...
  constructor(config, defaultConfig = {}) {
    if (this.config.var && varKeywords.indexOf(this.config.var) !== -1) {
      throw `Your variable ${this.config['var']} must be not in keywords ${varKeywords.join(',')}`;
    }
    if (config.extends) {
  ...
  applyType(a) {
    if ('function' === typeof a) {
      a = `$(${a.name}${a.more || ''})`;
    } else if (a && a instanceof Object || a instanceof Array) {
      for (var i in a) {
        a[i] = this.applyType(a[i]);
      }
    }
    return a;
  }
    ...
    if (this.config.checker) {
      for (var i in this.config.checker) {
        ...
        if (checkerObj._apply !== undefined) {
          ...
          checkerObj[checkerName] = utils.merge(checkerObj[checkerName], _applyObject);
        ...
        utils.mes = [];
        checkerFnc(res.code, res.headers, res.body, checkerObj[checkerName], opts);
        // }catch(e){
        //   if(!checkerFnc) throw {
  ...
  exec(cb0) {
    this.config.index = ++Api.index;
    this.config.url = this.applyValue(this.config.url);

    if (this.config.index === 42) {
      debugger;
    }
    ...
    req.end(res => {
      ...
      try {
        if ('test' === global.mode) {
          self.check(res);
          if (self.config.checker) self.config.checker = self.applyType(self.config.checker);
        }
