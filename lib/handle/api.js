  ...
  check(res) {
    if (this.config.checker) {
      for (var checkerObj of this.config.checker) {
        ...
        var checkerMatching = this.config.apply ? this.config.apply(checkerObj[checkerName]) : checkerObj[checkerName];
        checkerFnc(res.code, res.headers, res.body, checkerMatching);
