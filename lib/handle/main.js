...
module.exports = class Main {
  ...
  constructor(pathProject) {
    ...
    if(this.config.apiTemplates instanceof Array){
      for (var i in this.config.apiTemplates) {
        new TestCase(require(`../${pathProject}/testcases/templates/${this.config.apiTemplates[i]}.api`), this.config.defaultInit.testcase, this.config.defaultInit.api);
      }
    }else if('*' === this.config.apiTemplates){
      var path = require('path');
      var files = fs.readdirSync(path.join(__dirname, `../${pathProject}/testcases/templates`));
    ...
    for (var i in this.config.testcases) {
      this.config.testcases[i] = require(`../${pathProject}/testcases/${this.config.testcases[i]}.api`);
    }
  ...
  getTest(cb){
    cb({mode: Main.mode, testcases: this.config.testcases, path: this.config.path, projectName: this.config.projectName, team: this.config.team});
  }
